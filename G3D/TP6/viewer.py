...
class KeyFrames:
    ...
    def value(self, time):
        ...
        k = (time-self.times[i])/(self.times[i+1]-self.times[i])
        ...
        return self.interpolate(self.values[i], self.values[i+1], k)
...
class TransformKeyFrames:
    ...
    def __init__(self, translate_keys, rotate_keys, scale_keys):
        ...
        self.translate_keys = KeyFrames(translate_keys, lerp)
        self.rotate_keys = KeyFrames(rotate_keys, quaternion_slerp)
        self.scale_keys = KeyFrames(scale_keys, lerp)

    def value(self, time):
        """ Compute each component's interpolation and compose TRS matrix """
        return(translate(self.translate_keys.value(time))
               @ quaternion_matrix(self.rotate_keys.value(time))
               @ scale(self.scale_keys.value(time)))
...
def main():
    ...
    rotate_keys = {0: quaternion(),
                   4: quaternion_from_euler(180, 45, 90),
                   8: quaternion_from_euler(180, 0, 180),
                   12: quaternion(),
                   16: quaternion()}
    scale_keys = {0: 1,
                  8: 0.5,
    ...
    node = KeyFrameControlNode(translate_keys, rotate_keys, scale_keys)
